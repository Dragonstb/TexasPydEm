...
class HandEvaluator_UT(UT.TestCase):
    ...
    def test_checkThreeOAK_one_Three(self):
        ...
        self.assertEqual(1, len( pairs ), 'wrong number of threes of a kind detected')
    ...
    def test_checkThreeOAK_twoThrees(self):
        ...
        freqs = HE.getValueFreqs( cards )
        pairs = HE.checkMultiples( freqs, 3 )
        self.assertEqual(2, len( pairs ), 'wrong number of threes of a kind detected')
        self.assertTrue( valA in pairs, 'valA not detected' )
        self.assertTrue( valB in pairs, 'valB not detected' )
    ...
    def test_checkFourOAK_one_Three(self):
        ...
        self.assertEqual(1, len( pairs ), 'wrong number of fours of a kind detected')
        self.assertTrue( val in pairs, 'val not detected' )
    ...
    def test_checkStraight_oneStr(self):
        ...
        self.assertTrue(val in strgt, 'straight does not ends with expected card')
    ...
    def test_checkStraight_threeStr(self):
        ...
        self.assertTrue(val in strgt, 'straight does not ends with expected card')
        self.assertTrue(val+1 in strgt, 'straight does not ends with expected card')
        self.assertTrue(val+2 in strgt, 'straight does not ends with expected card')

    def test_checkStraight_noStrCosNeedSixCards(self):
        val = 10
        cards = [val, val-1, 1, val-2, 3, val-3, val-4] # a five-card straigh ...
        freqs = HE.getValueFreqs( cards )
    ...
    def test_checkFlush_noFlush(self):
        ...
        self.assertEqual( len(flush), 0, 'flush detected, freqs are '+self.asStr( freqs ))
    ...
    def test_checkFlush_aFlush(self):
        ...
        freqs = HE.getSuitFreqs( cards )
        flush = HE.checkFlushes( freqs )
        self.assertEqual( len(flush), 1, 'flush missed, freqs are '+self.asStr( freqs ))
    ...
    def test_otherValues_1(self):
        ...
        kickers = [11, 8, 5]
        self.assertEqual( len(others), len(kickers), 'other values have wrong length')
        for idx in range( len(others) ):
            self.assertEqual( others[idx], kickers[idx], 'other value not in place')


    def test_otherValues_kickNone(self):
        cards = [11, 8, 5, 4, 4+13]
        others = HE.getCardsOfOtherValue( cards, [1])
        self.assertEqual( len( cards ), len( others ), 'not all cards detected: '+self.asStr(others))
        for idx in range( len(cards) ):
            self.assertEqual( others[idx], CU.getCardValue( cards[idx] ), 'incorrect value')
    ...
    def test_eval_highCard(self):
        ...
        for h in range( len(HandEvaluator_UT.hands) ):
            if h == idx:
                ...
                self.assertNotEqual( combi[0], h, 'wrong combination detected: '+str(h) )
        vals = sorted(HandEvaluator_UT.values[idx], reverse=True)
        for kick in range(5):
            self.assertEqual( combi[kick+1], vals[kick], 'incorrect kicker: '+str(kick) )
    ...
    def test_eval_pair(self):
        ...
        for kick in range(3):
            self.assertEqual( kickers[kick], combi[kick+2], 'incorrect kicker: '+str(kick) )
    ...
    def test_eval_flush(self):
        ...
        vals = [12, 11, 9, 5, 3]
        for val in range(5):
            self.assertEqual( combi[val+1], vals[val], 'incorrect value at '+str(val) )
